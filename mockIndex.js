...
const {Hedera} =  require( './hedera.js');
 ...
 class PolicyNetwork {
   ...
   constructor(hiddenLayerSizesOrModel) {
       // this might affect performance if loading a saved model
       // as it needs to hold the current aggregated tensor values and num of aggregations in the this.h.agg.currentAggregatedTensor
        //this.h = new Hedera();
        //instanciate training data output
   ...
   async train(
     ...
     for (let i = 0; i < numGames; ++i) {
       ...
       //update training_data json object per game with score, game num, iteration
       //game steps
       

       console.log(gameRewards)
 ...
 const MODEL_SAVE_PATH_ = 'indexeddb://local-model-1';
  ...
  */
 //export here and delete module.exports below to original
 export class SaveablePolicyNetwork extends PolicyNetwork {
   ...
   setWorkerUserId(worker_user_id){
       this.h.user_worker_id = worker_user_id;
   }

   getWorkerUserId(){
       return this.h.user_worker_id;
   }

   async maybeUpdateModel(meansteps){
    // await for comparison to be made, where if aggregation is done, a new local-model file will be made
    const result = await this.h.compareStepsWithLedger(meansteps);
    // if aggregation has been done, load new model from saved model files
    if (result == 1)
        {
            // load from indexxed db, when federate function loads and saves to there
            this.policyNet = await tf.loadLayersModel(LOCAL_MODEL_SAVE_PATH_);
        }
    return result;
   }
   ...
   async saveModel() {
     ...
     return await this.policyNet.save(LOCAL_MODEL_SAVE_PATH_);
   }
   ...
   static async loadModel() {
     ...
     if (LOCAL_MODEL_SAVE_PATH_ in modelsInfo) {
       console.log(`Loading existing model...`);
       const model = await tf.loadLayersModel(LOCAL_MODEL_SAVE_PATH_);
       console.log(`Loaded model from ${LOCAL_MODEL_SAVE_PATH_}`);
       //insert aggregated tensor, num of aggregations ?
       return new SaveablePolicyNetwork(model);
     } else {
       throw new Error(`Cannot find model at ${LOCAL_MODEL_SAVE_PATH_}.`);
     }
   }
   ...
   static async checkStoredModelStatus() {
     const modelsInfo = await tf.io.listModels();
     return modelsInfo[LOCAL_MODEL_SAVE_PATH_];
   }
    ...
    */
   async removeModel() {
     return await tf.io.removeModel(LOCAL_MODEL_SAVE_PATH_);
   }
