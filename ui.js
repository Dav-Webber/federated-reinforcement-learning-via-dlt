...
import { json } from 'stream/consumers';
 ...
 //change to index, mockIndex is just for testing
 import {SaveablePolicyNetwork} from './mockIndex';
 import {mean, sum} from './utils';
 //import {HCS} from  './hedera';
 ...
 const trainReward = document.getElementById('train-reward');
 const stepsContainer = document.getElementById('steps-container');

 const worker_user_id = document.getElementById('num-worker-id');
 ...
 export function onGameEnd(gameCount, totalGames) {
   ...
   //after each iteration pause and decide whether to publish gradients depending on what has already been published? on game end or iteration end?
   //after each iteration pause and decide whether to retrieve and aggregate other worker models based on their score? on game end or iteration end?
 }
 ...
 function onIterationEnd(iterationCount, totalIterations) {
   ...
   //after each iteration pause and decide whether to publish gradients depending on what has already been published
   //after each iteration pause and decide whether to retrieve and aggregate other worker models based on their score
 }
 ...
 export async function setUpUI() {
   ...
   //instanciates variable conecting to HCS class
   //const hcs = new HCS(true);
   //makes crypto transfer and logs receipt (test to see if works only)
   //await hcs.transferCrypto();
   ...
   trainButton.addEventListener('click', async () => {
     if (trainButton.textContent === 'Stop') {
       ...
       try {
        ...
        //get worker user id inputted from the user
        const worker_user_id_input =  worker_user_id.value;
        //sets an identifier, used for separating out different workers
        policyNet.setWorkerUserId(worker_user_id_input);
        ...
        let js_data = [];
         ...
         for (let i = 0; i < trainIterations; ++i) {
           ...
           t0 = t1;
           trainSpeed.textContent = `${stepsPerSecond.toFixed(1)} steps/s test`;
           meanStepValues.push({x: i + 1, y: mean(gameSteps)});
           console.log(`# of tensors: ${tf.memory().numTensors}`);
           trainReward.textContent =  `${mean(gameSteps).toString()} average steps per game for iteration :  ${i + 1} `
           plotSteps();
           onIterationEnd(i + 1, trainIterations);
           
           //TODO: generate json row object and push to json_data
           console.log('create row obj');
           let row_obj = {"iteration": i, "mean_score": mean(gameSteps), "worker": worker_user_id_input };
           //push row
           console.log('push row obj');
           js_data.push(row_obj);
           console.log(js_data);


           await tf.nextFrame();  // Unblock UI thread.
           await policyNet.saveModel();
           //will federate and save model again if required, else will nothing
           console.log('Checking Ledger to see if any aggregations will take place!');
           logStatus('Listening to Hedera and Checking if Aggregation is Required...');
           const maybeUpdateResult = await policyNet.maybeUpdateModel(mean(gameSteps));
           if (maybeUpdateResult == 1)
           {
            logStatus('Model Aggregated from Web3.Storage Completed');
           }
         ...
         var json_data = JSON.stringify(js_data);
         console.log('creating file object with training data');
         var data_file = new File([json_data], `${worker_user_id_input}_training_data.json`, {type:'application/json'});
         var files_for_upload = [data_file];
         console.log('Uploading training data files to Web3.Storage...');
         const cid = await policyNet.h.storage.client.put(files_for_upload);
   ...
   testButton.addEventListener('click', async () => {
     ...
     let weights_cid = 'bafybeiezaiehjzsxlghtqsginbjraxozafl4ufjho3zsnp7lt6los6oiem';
     let json_cid = 'bafybeiec4ts367kzzgmenswa7elsoxbpum4iba4nk7sbz4fopdmocmo5ge';
     await storage.downloadGlobalModel(weights_cid, json_cid);
     console.log('success');
     // test finished
     disableModelControls();
     let isDone = false;
